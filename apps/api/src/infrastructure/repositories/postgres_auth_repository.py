...
class TenantRepository:
    ...
    async def get(self, tenant_id: UUID) -> Tenant | None:
        model = await self.db.get(TenantModel, tenant_id)
...
class UserRepository:
    ...
    async def get_by_email(self, email: str) -> User | None:
        ...
        model = await self.db.scalar(select(UserModel).where(UserModel.email == email))
        return user_to_domain(model) if model else None

    async def get_by_google_sub(self, google_sub: str) -> User | None:
        # `google_sub` is globally unique, so one Google account maps to exactly one user.
        model = await self.db.scalar(select(UserModel).where(UserModel.google_sub == google_sub))
        return user_to_domain(model) if model else None
    ...
    async def link_google(self, user_id: UUID, google_sub: str) -> User:
