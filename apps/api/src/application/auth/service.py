...
Tenant resolution is credential-based: a login presents an email and a password, nothing
else. `users.email` is globally unique, so the user is resolved first and their tenant
follows from them. The issued access token carries `tid`/`sub`, so every later request
knows its tenant with no DB lookup.
...
def _default_workspace_name(email: str) -> str:
    """Fall back to the email's local part when no workspace name is supplied."""
...
class AuthService:
    ...
    def register(self, email: str, password: str, name: str | None = None) -> AuthTokens:
        ...
        email's local part. Nothing about it needs to be unique.
        """
        ...
        display_name = (name or "").strip() or _default_workspace_name(email)
    ...
    def login(self, email: str, password: str) -> AuthTokens:
        ...
        `InvalidCredentialsError`; the specific reason is only logged server-side. In
        particular an unknown email and a wrong password are indistinguishable to the
        caller.
        """
        email = _normalize_email(email)
