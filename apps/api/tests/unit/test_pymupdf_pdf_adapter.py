...
class PyMuPDF4LLMAdapterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_converts_multi_page_pdf_to_markdown_with_page_locators(self) -> None:
        pdf_bytes = self._sample_pdf_bytes(
            ["First page body", "Second page body"], title="Sample Title"
        )
        adapter = PyMuPDF4LLMAdapter()

        parsed = await adapter.load(pdf_bytes, "sample.pdf")
    ...
    async def test_pdf_without_a_title_falls_back_to_none(self) -> None:
        pdf_bytes = self._sample_pdf_bytes(["Body text"])
        adapter = PyMuPDF4LLMAdapter()

        parsed = await adapter.load(pdf_bytes, "untitled.pdf")

        self.assertIsNone(parsed["title"])

    async def test_placeholder_metadata_title_falls_back_to_none(self) -> None:
