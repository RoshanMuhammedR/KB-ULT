...
class FilebaseAdapterTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    async def test_upload_returns_existing_key(self, client_factory: Mock) -> None:
        client = Mock()
        client_factory.return_value = client
        adapter = FilebaseAdapter(self.settings)

        key = await adapter.upload("user/asset/file.pdf", b"pdf", "application/pdf")
    ...
    @patch("src.infrastructure.storage.filebase_adapter.boto3.client")
    async def test_presigned_url_uses_get_object(self, client_factory: Mock) -> None:
        client = Mock()
        client.generate_presigned_url.return_value = "https://signed"
        client_factory.return_value = client
        adapter = FilebaseAdapter(self.settings)

        url = await adapter.get_presigned_url("user/asset/file.pdf", 60)
    ...
    @patch("src.infrastructure.storage.filebase_adapter.boto3.client")
    async def test_delete_wraps_storage_errors(self, client_factory: Mock) -> None:
        client = Mock()
        ...
        with self.assertRaises(FileStorageError):
            await adapter.delete("user/asset/file.pdf")
